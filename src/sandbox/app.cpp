...
App::App() :
		context_(3, 3) {
	if (!InitWindow()) {
		throw std::runtime_error("Fail to init glfw window");
	}
	// init glew
...
int App::Run() {
	...
	std::string_view frag_shader_text(frag_shader.cbegin(), frag_shader.size());
	std::string_view vertex_shader_text(vertex_shader.cbegin(), vertex_shader.size());
